...
function addOpeningClassToTokens(tokens) {
  var i,
    hangingCharsRegex = "'|\"",
    optionalClassRegex = classesRegex.replace("+", "*?");

  for (i = 0; i < tokens.length; i += 1) {
    // paragraphs with a class of "opening" get a cap added to them
    if (tokens[i].type.indexOf("paragraph") > -1
        && tokens[i].text.match(new RegExp("\\{" + optionalClassRegex + "opening" + optionalClassRegex + "\\}$"))) {
