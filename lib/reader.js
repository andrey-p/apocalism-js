...
exports.readStandalonePage = function (pageName, callback) {
  function checkedForImageFile(err) {
    ...
    images.resolveImageTag(markup, pathToImages, callback);
  }
  ...
  function readMarkdownFile(err, markdown) {
    ...
    if (err) {
      ...
      fs.stat(pathToImages + "/" + pageName + ".png", checkedForImageFile);
      return;
    }

    markup = html.prepMarkup(namp(markdown).html);

    images.resolveImagesInMarkup(markup, pathToImages, callback);
  }
