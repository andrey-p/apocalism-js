...
exports.saveHtml = function (pageIndex, contents, callback) {
  fs.writeFile(options.pathToCache + "/html/" + pageIndex + ".html",
    contents,
    callback);
};
...
exports.getCachedPdfs = function (callback) {
  async.waterfall([
    function (callback) {
      fs.readdir(options.pathToCache + "/pdf/", callback);
    },
    function (filenames, callback) {
      ...
      filenamesWithPath = filenames.map(function (filename) {
        return options.pathToCache + "/pdf/" + filename;
      });
...
exports.checkIfHtmlChanged = function (pageIndex, contents, callback) {
  async.series({
    html: function (callback) {
      fs.readFile(options.pathToCache + "/html/" + pageIndex + ".html",
        { encoding: "utf8" },
        callback);
    },
    pdf: function (callback) {
      fs.readFile(options.pathToCache + "/pdf/" + pageIndex + ".pdf",
        callback);
    }
  }, function (err, result) {
...
exports.init = function (callback) {
  async.reject([options.pathToCache, options.pathToCache + "/html", options.pathToCache + "/pdf"],
    fs.exists,
